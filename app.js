...
async function staticDemoApi(path, options = {}) {
  const demo = await loadStaticDemoData();
  ...
  if (pathname === "/auth/login" && method === "POST") {
    const username = String(body.username || "superadmin").trim() || "superadmin";
    const user = demo.users.find((item) => item.username === username) || demo.users.find((item) => item.username === "superadmin");
    return {
      access_token: "slate-static-demo-token",
      token_type: "bearer",
      expires_at: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
      user,
    };
  }
  if (pathname === "/auth/me") return demo.users.find((item) => item.username === "superadmin");
  if (pathname === "/auth/logout" && method === "POST") return null;
  if (pathname === "/modules") return demo.modules;
  if (pathname === "/framework") return demo.framework;
  if (pathname === "/cases") return demo.cases;
  if (pathname === "/cases/search") return staticCaseSearch(demo, url.searchParams);
  if (pathname === "/files/uploads") return [];
  if (pathname === "/analytics/trends") return demo.trends;
  if (pathname === "/analytics/patterns") return demo.patterns;
  if (pathname === "/analytics/advanced-crime") return demo.advanced_crime;
  if (pathname === "/analytics/network") return demo.network;
  if (pathname === "/analytics/sociological") return demo.sociological;
  if (pathname === "/financial/analysis") return demo.financial;
  if (pathname === "/forecast/hotspots") return demo.forecast;
  if (pathname === "/audit/integrity") return demo.audit_integrity;
  if (pathname === "/audit/logs") return demo.audit_logs;
  if (pathname === "/admin/users") return demo.users;
  if (pathname === "/admin/rate-limit-alerts") return [];
  if (pathname === "/penal-codes") return demo.penal_codes;
  if (pathname === "/crime-logs" && method === "POST") {
    return { id: `slate-crime-log-${Date.now()}`, status: "logged", demo_mode: true };
  }
  if (pathname === "/conversations" && method === "POST") return staticConversation();
  if (/^\/conversations\/[^/]+\/messages$/.test(pathname) && method === "POST") {
    return staticConversationExchange(demo, body.query || "");
  }
  if (pathname === "/translate" && method === "POST") {
    return { translated_text: body.text || "", source_language: "en", target_language: "kn", adapter: "slate-static-demo" };
  }
  throw new Error("This Slate static demo action needs the AppSail backend.");
}
...
function staticCaseSearch(demo, params) {
  const query = String(params.get("q") || "").toLowerCase();
  const limit = Number(params.get("limit") || 25);
  const started = performance.now();
  const results = demo.cases
    .filter((item) => JSON.stringify(item).toLowerCase().includes(query))
    .slice(0, limit)
    .map((item) => ({ case: item, score: 1 }));
  return {
    query,
    result_count: results.length,
    elapsed_ms: Math.max(0.1, performance.now() - started),
    results,
  };
}
...
function staticConversation() {
  return {
    id: `slate-demo-${Date.now()}`,
    title: "Slate demo conversation",
    created_at: new Date().toISOString(),
  };
}

function staticConversationExchange(demo, query) {
  const intelligence = staticIntelligenceForQuery(demo, query);
  const now = new Date().toISOString();
  return {
    conversation: staticConversation(),
    user_message: { role: "user", content: query, created_at: now },
    assistant_message: { role: "assistant", content: intelligence.answer, created_at: now },
    intelligence,
  };
}

function staticIntelligenceForQuery(demo, query) {
  const normalized = String(query || "").toLowerCase();
  if (normalized.includes("forecast") || normalized.includes("next 7") || normalized.includes("hotspot")) {
    return demo.chatbot_responses.forecast;
  }
  if (normalized.includes("money") || normalized.includes("financial") || normalized.includes("account")) {
    return demo.chatbot_responses.financial;
  }
  if (normalized.includes("network") || normalized.includes("pooja")) {
    return demo.chatbot_responses.network;
  }
  if (normalized.includes("pattern") || normalized.includes("district")) {
    return demo.chatbot_responses.patterns;
  }
  if (normalized.includes("ravi") || normalized.includes("fir") || normalized.includes("case")) {
    return demo.chatbot_responses.case_lookup;
  }
  return demo.chatbot_responses.overview;
}
  ...
  return `
    <div class="assistant-meta">
      <details class="assistant-evidence">
        <summary>
          <span><i data-lucide="search"></i>Evidence Sources (${escapeHtml(formatCount(evidenceCount))})</span>
